   ...
   class GeoLocationService {
    ...
    class ContentRenderer {
        ...
        renderSidebar(sidebarDiv, sidebarContent) {
            const sidebarLinksHtml = sidebarContent.links.map(link => `
                <a href="${link.href}"><img class="sidebar-icon" src="${link.imgSrc}" alt="${link.altText}"> ${link.text}</a>
            `).join('');

            const bottomLinksHtml = sidebarContent.bottomLinks.map(link => `
